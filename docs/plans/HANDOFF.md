...
## Latest milestone (2026-07-26): engine package, #57 stage 1 first slice (v0.12.0)
...
**Next:** #59 (stage 0 — strip write authority from the legacy model steps; the
injection→mutation hole should not wait for stage 4), then #61 (acceptance fixture) and #62
(report contract with lineage and the five result states), both of which build directly on the
inventory. `gh issue list --label ready-for-agent` has the full sequence with its blocked-by
edges.

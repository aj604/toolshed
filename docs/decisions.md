...
- Decided: `record_verdicts()` treats those three codes as whole-document judgments and requires
  ...
  the existing unknown-unit refusal covers extra, stale, or malformed identities. The check runs
  before a finding is built, so an incomplete judgment cannot become report, approval-set, or
  applier authority.
...
- Verified: focused public-seam regressions cover all three omission refusals before report
  production, duplicates and foreign/stale identities, complete records through human approval and
  deterministic application, unchanged passage remedies, and deterministic bulk retirement.
...
- Code: plugins/doc-lifecycle/engine/doclifecycle/bloat.py,
  ...
  tests/engine/bloat_test.py, tests/engine/applier_test.py

...
| `units` | non-empty array of **unit digests** — the `digest` values `python3 -m doclifecycle segment --repo . --path <path>` prints for that document, copied verbatim. This is what the verdict is *about*, and what the applier's span is bounded by. Required unless the verdict carries `scope`. **`CUT`/`CONDENSE`/`EXTRACT-AND-MOVE`** (a passage verdict): name only the passage's own units. **`DISTILL`/`MERGE-DOC`/`RETIRE-DOC`** (a whole-document verdict, subject is the document, not a passage): name **every distinct** unit `segment` prints for it, assertion-capable or not — never one representative, never only the assertion-capable subset, and never the same identity twice. The engine enforces exact equality with the document's complete current deterministic unit set before it builds a finding: omissions are `bloat-whole-document-units-incomplete`, duplicates are `bloat-whole-document-unit-duplicate`, and an extra or stale identity is `bloat-unknown-unit`. Repository-backed report validation, approval minting/read-back, and the applier re-derive the same invariant, so a forged or legacy partial record never gains whole-document authority. The applier may retire the source only because this record already binds everything that retirement removes |

...
- **api listens on port `3000` by default** (`services/api/server.js:21`); override with `PORT`.
- **Worker only accepts state schema 2.** A stale migration makes it exit `5` (`services/worker/worker.js:17`).
...
- `services/api/server.js` — HTTP API (`/health`, `POST /tasks`); a successful `POST /tasks` returns `200`. In-memory `tasks`, no persistence.
- `services/worker/worker.js` — polls on an interval; reads state schema only.
- `packages/shared/index.js` — `makeId`, `validateTask`, `normalizePriority`, `formatTask`; used by both services.
- `scripts/setup-db.js` — stamps `.taskflow-state.json`. `scripts/lint.js` — parse-check only, not eslint.

...
| P2 | `fixture/README.md:19` ("The `send_alert` function takes a channel and a message and delivers the message to that channel.") | `CUT` | Restates what `fixture/src/notify.py:7` (`def send_alert(channel, msg):`) already shows on its face — the signature is self-evident from the code one line away in the same doc's usage example. No new information. |
